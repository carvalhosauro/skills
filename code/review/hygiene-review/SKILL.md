...
name: hygiene-review
description: >-
  Runs a readability- and session-aftermath hygiene review of recently written
  code by dispatching parallel subagents, one per category: magic values, dead
  code, duplication, N+1 queries, missing typing, general readability,
  error-handling, boundaries (I/O/HTTP/DB), and secrets-and-config. It only
  REPORTS — it never edits code. Use this skill AFTER large changes or long
  agent coding sessions, before opening a PR, or when the user asks to "review
  what we just wrote", "audit after this session", "clean up the big change",
  "find code smells", or "go over the diff". Do NOT use for every micro-edit —
  the default trigger is post-session / post-large-diff. Trigger even without
  the word "skill".
---
...
chosen scope. The single goal is **readability and session aftermath**:
surfacing the smells that accumulate during long agent sessions so a human can
decide what to fix.

**When to run (default):** after a long coding session, after the agent made
many modifications, or before a PR on a large diff. Skip for tiny one-file
tweaks unless the user explicitly asks.
...
- **Session** — code changed in the current working session. Resolve this from
  ...
  to name the files or a path instead. **Prefer this default** after long sessions.
...
Launch **nine** subagents in a single turn using the Task tool. Each one owns one
...
{ "file": "...", "line": <int or null>, "severity": "high|medium|low",
  "title": "short label", "why": "why it hurts readability or safety",
...
The nine categories and their checklists:
...
| Error handling      | `references/checks/error-handling.md`     |
| Boundaries          | `references/checks/boundaries.md`         |
| Secrets and config  | `references/checks/secrets-and-config.md` |
...
- **Readability (and shallow safety) is the lens.** Every finding answers "why
  does this make the code harder to read, maintain, or safely operate?" If a
  finding can't answer that, drop it.
...
- **Post-session, not continuous.** Default use is after large diffs / long
  sessions — not after every small edit.

## Scope notes

This skill deliberately stays on shallow, session-aftermath smells. It does
**not** cover tests, logging depth, dependency injection, or deep architecture —
those belong to other skills (`behavior-tests`, etc.).
